CREATE OR REPLACE PROCEDURE `main`.`gold`.`sp_load_sales_enterprise1`()
LANGUAGE SQL
...
BEGIN
    ...
    SELECT V_ExecutionID, V_EntityName, V_CurrentLoadDate, 'RUNNING';
    ...
    ******************************************************************************************/
    ...
    SELECT
        ...
        DATEDIFF(MAX(sh.OrderDate), V_CurrentLoadDate) AS Recency,
        COUNT(sh.SalesOrderID) AS Frequency,
        SUM(sh.TotalDue) AS Monetary,
        V_CurrentLoadDate AS LoadDate
    FROM main.silver.customer c
    LEFT JOIN main.silver.sales_salesorderheader sh
    ...
    CREATE OR REPLACE TABLE gold.stg_customer_segment AS
    SELECT
    ...
    DROP TEMPORARY TABLE IF EXISTS TEMP_TABLE_stg_customer_segment;
    
    CREATE TEMPORARY TABLE TEMP_TABLE_stg_customer_segment AS
    SELECT
        cs_CustomerID,
        Segment_Recency,
        Segment_Frequency,
        ...
        RFM_Score,
        LoadDate
    FROM main.gold.stg_customer_segment;
    
    MERGE INTO main.gold.fact_customer_segment AS T
    USING TEMP_TABLE_stg_customer_segment AS S
    ON T.CustomerID = S.cs_CustomerID
    WHEN MATCHED THEN UPDATE SET Segment_Recency = S.Segment_Recency,
    Segment_Frequency = S.Segment_Frequency,
    Segment_Monetary = S.Segment_Monetary,
    RFM_Score = S.RFM_Score,
    LoadDate = S.LoadDate
    WHEN NOT MATCHED THEN INSERT (CustomerID, Segment_Recency, Segment_Frequency, Segment_Monetary, RFM_Score, LoadDate)
    ...
    ----------------------------------------------------------------------------------------
    DROP TABLE IF EXISTS main.gold.stg_sales_territory_kpi;
    
    CREATE OR REPLACE TABLE main.gold.stg_sales_territory_kpi AS
    SELECT
        ...
        V_CurrentLoadDate AS LoadDate
    FROM main.silver.sales_salesorderheader s
    JOIN main.silver.sales_salesorderdetail d
    ...
    ----------------------------------------------------------------------------------------
    MERGE INTO main.gold.fact_sales_territory AS T
    USING main.gold.stg_sales_territory_kpi AS S
    ON T.TerritoryID = S.t_TerritoryID AND T.ProductID = S.t_ProductID
    WHEN MATCHED THEN UPDATE SET TotalRevenue = S.TotalRevenue,
    TotalQty = S.TotalQty,
    AvgUnitPrice = S.AvgUnitPrice,
    NumSalesPersons = S.NumSalesPersons,
    LoadDate = S.LoadDate
    WHEN NOT MATCHED THEN INSERT (TerritoryID, ProductID, TotalRevenue, TotalQty, AvgUnitPrice, NumSalesPersons, LoadDate)
    ...
    ----------------------------------------------------------------------------------------
    DROP TABLE IF EXISTS main.gold.stg_customer_rank;
    
    CREATE OR REPLACE TABLE main.gold.stg_customer_rank AS
    SELECT
        CustomerID,
        TerritoryID,
    ...
    ******************************************************************************************/
    ...
    ----------------------------------------------------------------------------------------
    DROP TABLE IF EXISTS main.gold.stg_sales_advanced_kpi;
    
    CREATE OR REPLACE TABLE main.gold.stg_sales_advanced_kpi AS
    SELECT
        ...
        V_CurrentLoadDate AS ak_LoadDate
    FROM main.gold.stg_sales_running s
    LEFT JOIN main.gold.stg_product p ON s.s_ProductID = p.p_ProductID
    LEFT JOIN main.gold.stg_sales_header sh ON s.s_ProductID = sh.sh_SalesOrderID
    ...
    ----------------------------------------------------------------------------------------
    MERGE INTO main.gold.fact_sales_advanced AS T
    USING (
        SELECT
            ak_ProductID,
            ak_TotalQty,
    ...
    WHEN NOT MATCHED THEN INSERT (ProductID, OrderYear, OrderMonth, TotalQty, TotalRevenue, GrossMargin, OnlineRevenue, OfflineRevenue, LoadDate)
    ...
    DROP TABLE IF EXISTS main.gold.stg_customer_rfm;
    DROP TABLE IF EXISTS main.gold.stg_customer_segment;
    DROP TABLE IF EXISTS main.gold.stg_sales_kpi;
    DROP TABLE IF EXISTS main.gold.stg_sales_territory_kpi;
    DROP TABLE IF EXISTS main.gold.stg_sales_advanced_kpi;
    DROP TABLE IF EXISTS main.gold.stg_sales_groupings;
